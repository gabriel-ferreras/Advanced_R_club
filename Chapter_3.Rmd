...
c(1, FALSE) will create a (1,0) double vector.
c("a", 1) will create a ("a", "1") character vector.
c(TRUE, 1L) will create a (1L, 1L) integer vector.
...
1 == "1" is TRUE because 1 is coerced into "1".
-1 < FALSE is TRUE because FALSE is coerced into 0.
"one" < 2 because 2 is coerced into "2" which cannot be compared against "one".
...
f2 has the levels in the original order but the vector inverted.
f3 has the levels in the reverse order but the vector in right order.
